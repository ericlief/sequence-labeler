    ...
    def construct(self, args, n_tags, restore_model, model_path, char_dict_path):
        ...
        with self.session.graph.as_default():
            ...
            if args.use_words:
                ...
                word_embedding = tf.get_variable("word_embedding", [n_words, args.rnn_dim], tf.float32)
                embedded_words = tf.nn.embedding_lookup(word_embedding, self.words)
            ...
            if args.use_lemmas:
                ...
                embedded_lemmas = tf.nn.embedding_lookup(pos_tag_embedding, self.lemmas)
            ...
            # Normalization
            if args.bn_sent:
                inputs = tf.layers.batch_normalization(inputs, training=self.is_training, name="bn_sent")
            elif args.dropout_sent:
                inputs = tf.nn.dropout(inputs, 1 - args.dropout_sent, name="dropout_sent")
    ...
    def train(self, 
        ...
        for epoch in range(args.epochs):
            ...
            for batch_n, batch in enumerate(batches):
                ...
                if args.use_words:
                    words = np.zeros([n_sents, max_sent_len])                 
                if args.use_pos_tags:
