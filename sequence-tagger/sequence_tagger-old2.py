...
class SequenceTaggerTrainer:
    ...
    def train(self, batch, batch_size):
        ...
        # Reset batch metrics
        model.session.run(model.reset_metrics)
        
        # Embed sentences
        model.embedding.embed(batch)        
        ...
        _, _, loss, predictions = model.session.run([self.training, self.summaries["train"], self.loss, self.predictions],
